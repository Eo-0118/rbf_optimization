...
### 본 연구의 운영 시스템 (Day 11+ 핵심 결과)

**(L\*, T\*, cap\*) 사전 적합도 평가 + 적합 셀러 다중 정책 선택**
...
→ 한국 영세 셀러 약 88% 거절 (m_i=0.10 기준). 적합 12-13%는 스마트스토어 매출 상위 셀러 분포 부합.

### 적합 셀러 160명에 (L\*, cap\*) 적용 시 정책별 성능

| 정책 | Completion | 단일월 최대 침해비 | 강점 |
|---|---|---|---|
| Fixed-0.15 | **100%** | 2.34 | 회수 우위, 침해 큼 |
| **CVaR** | **86.9%** | **0.92** | **회수+안전 균형** ⭐ |
| PPO base | 52.5% | 1.81 | 중간 |
| **PPO x1 비례** | 32.5% | **0.21** | **안전 절대 우위** ⭐ |

### Lv4 추가 발견 — RL 9가지 시도 (정직한 부정 결과)

본 연구는 동적 r 가치 검증을 위해 PPO/SAC 9가지 변형을 시도했으나, 본 합성 환경에서 모두 정적 CVaR을 능가하지 못함:

| Algorithm | 변형 | R_p10 81명 Completion | 단일월 최대비 |
|---|---|---|---|
| 정적 (CVXPY) | **CVaR** | **100%** | **0.99** ⭐ |
| PPO | base | 50.6% | 1.61 |
| PPO | eta_strong (η=5) | 32.5% | 0.22 |
| PPO | x1_only (비례 페널티) | 44.4% | **0.13** |
| PPO | x1+x2 | 0% | 0.20 |
| PPO | forecast 통합 | 50% 내외 | 1.7 내외 |
| PPO | v2 (state + multi) | 44.4% | 0.13 |
| PPO | v2b (state + multi, X1 X) | 65.4% | 2.94 |
| PPO | cvar_hybrid (CVaR + delta) | 100% | 2.57 |
| **SAC** | (off-policy) | 97.5% | **4.50** |

**핵심 관찰**: 모든 동적 RL은 회수율 vs 침해 강도 trade-off에 빠짐. 정적 CVaR이 시나리오 기반 보수성으로 양쪽 균형 최강.

**학술적 의미**: 본 연구는 단순 RL 우위 주장이 아닌 **"본 합성 환경에서 정적 CVaR의 우위를 9가지 알고리즘 비교로 정량 입증"** — 학술적 정직성 + 의미 있는 부정 결과.

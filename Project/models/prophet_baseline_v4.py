"""Phase 2: Prophet 베이스라인 v4 — 카테고리별 Naver 매칭

v3 대비 변경 (단일):
  cohort_kr_v3 → cohort_kr_v4 사용 (naver_index가 셀러 카테고리별로 다름)

비교 가능성:
  - v3와 동일한 SAMPLES_PER_TYPE=50, train/val/test 분할 동일, SEED 동일
  - 외생변수 컬럼명도 동일 (naver_index, promo)
  - → 차이는 오직 naver_index 값 (글로벌 → 카테고리별)

산출:
  Data/prophet_baseline_v4_results.csv
  Data/prophet_baseline_v4_summary.json
  Data/prophet_baseline_v4_diagnostics.png
  Data/prophet_v3_vs_v4_comparison.png
"""
...
def load_cohort():
    df = pd.read_parquet(DATA / "cohort_kr_v4.parquet")
...
def main():
    print("[1/5] Cohort v4 로드 (카테고리별 Naver 매칭)")
    df = load_cohort()
    ...
    print("\n[4/5] 결과 분석 + 저장")
    res_df.to_csv(DATA / "prophet_baseline_v4_results.csv", index=False)
    ok = res_df[res_df["status"] == "ok"]

    summary = {
        "config": {
            "cohort": "cohort_kr_v4 (카테고리별 Naver 매칭)",
    ...
    (DATA / "prophet_baseline_v4_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False))

    print(f"\n=== Prophet Baseline v4 결과 (카테고리별 Naver) ===")
    ...
    # v3 vs v4 비교
    ax = axes[0, 2]
    v3_csv = DATA / "prophet_baseline_v3_results.csv"
    if v3_csv.exists():
        v3df = pd.read_csv(v3_csv)
        merged = v3df.merge(res_df, on="seller_id", suffixes=("_v3", "_v4"))
        merged_ok = merged[(merged["status_v3"] == "ok") & (merged["status_v4"] == "ok")].copy()
        ax.scatter(merged_ok["wape_test_v3"].clip(upper=200),
                   merged_ok["wape_test_v4"].clip(upper=200),
                   c=[color_map.get(t, "gray") for t in merged_ok["type_v4"]],
                   alpha=0.6, s=30)
        lim = 200
        ax.plot([0, lim], [0, lim], "k--", alpha=0.5, label="동일")
        improved_pct = (merged_ok["wape_test_v4"] < merged_ok["wape_test_v3"]).mean() * 100
        ax.set_xlabel("v3 WAPE (글로벌 Naver)")
        ax.set_ylabel("v4 WAPE (카테고리 Naver)")
        ax.set_title(f"v3 vs v4 ({improved_pct:.0f}% 개선)")
        ax.legend(); ax.grid(alpha=0.3)
    else:
        ax.text(0.5, 0.5, "v3 결과 없음", ha="center", va="center")
    ...
    plt.suptitle(f"Prophet v4 (cohort_kr_v4, 카테고리별 Naver, n={n_ok}, "
                 f"WAPE median {summary['overall']['wape_test_median']:.1f}%)",
                 fontsize=13, fontweight="bold", y=1.00)
    plt.tight_layout()
    plt.savefig(DATA / "prophet_baseline_v4_diagnostics.png", dpi=130, bbox_inches="tight")
    plt.close()
    print(f"\n[save] prophet_baseline_v4_diagnostics.png")
